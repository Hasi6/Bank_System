...
public class Bank {
    private String name;
    private ArrayList<Branch> branchers;

    public Bank(String name) {
        this.name = name;
        this.branchers = new ArrayList<Branch>();
    }
    public boolean addBranch(String branchName){
        if(findBranch(branchName)==null){
            this.branchers.add(new Branch(branchName));
            return true;
        }
        return false;
    }
    ...
    private Branch findBranch(String branchName){
        for (int i=0;i<this.branchers.size();i++){
            Branch checkedBranch = this.branchers.get(i);
